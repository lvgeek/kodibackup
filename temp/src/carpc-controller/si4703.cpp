...
int si_set_state(uint16_t *regs, const char *name, uint16_t val)
{
	...
	for(int i = 0; i < nstates; i++) {
		if (strcmp(si_state[i].name, name)) {
...
void si_set_channel(uint16_t *regs, int chan)
{
	...
	while(i++ < 100) {
		si_read_regs(regs);
		if (!(regs[STATUSRSSI] & STC))
		{
		    Radio_FrequencyUpdate();
		    rpi_delay_ms(100);
...
void si_mute(uint16_t *regs, int mode)
{
    if (!mode)
        regs[POWERCFG] |= DSMUTE | DMUTE; // unmute
    else
...
int si_get_volume(uint16_t *regs)
{
    return regs[SYSCONF2]&VOLUME;
}
...
int si_get_rssi(uint16_t *regs)
{

    return regs[STATUSRSSI] & RSSI;
}
